...
import com.apexfit.app.utils.AudioService
...
import com.apexfit.app.utils.PatternScanWorker
...
class TrainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun triggerRestTimer(rpe: Int, exerciseName: String, muscleGroup: String, reps: Int, setIndex: Int, totalSets: Int) {
        ...
        sessionManager.resetRestSeconds()
        ...
        com.apexfit.app.utils.RestTimerAlarmReceiver.scheduleAlarm(
    ...
    fun closeRestTimer() {
        ...
        _showRestOverlay.value = false
        restTimerJob?.cancel()
        // Commit the elapsed rest time to the last completed set
        lastCompletedSetPointer?.let { (exId, sIdx) ->
            sessionManager.commitRestTaken(exId, sIdx, sessionManager.currentRestSeconds.value)
        }
        sessionManager.resetRestSeconds()
    ...
    fun finishWorkoutSession(sessionFeel: Int = 4) {
        viewModelScope.launch {
            ...
            try {
                ...
                try {
                    val scanRequest = OneTimeWorkRequestBuilder<PatternScanWorker>()
                        .setConstraints(Constraints.Builder().setRequiresBatteryNotLow(false).build())
                        .build()
                    WorkManager.getInstance(getApplication()).enqueueUniqueWork(
                        "pattern_scan_after_session",
                        ExistingWorkPolicy.REPLACE,
                        scanRequest
                    )
                } catch (we: Exception) {
                    android.util.Log.e("TrainViewModel", "Failed to enqueue pattern scan", we)
                }
    ...
    private fun generateWorkoutSessionHypertrophyQualityScore(sets: List<UiExerciseSet>) {
        ...
        var scoreSum = 0.0
        sets.forEach { set ->
            val rpeMod = com.apexfit.app.utils.ProgressionEngine.calculateEffectiveSetValue(set.rpe)
